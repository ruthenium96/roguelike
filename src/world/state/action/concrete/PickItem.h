...
public:
    ...
    bool precondition(const object::Observer &observer,
                      const std::set<std::shared_ptr<AbstractAction>> &set) override;

    void changeTarget(object::Observer &observer,
                      std::set<std::shared_ptr<AbstractAction>> &set) override;
