...
GeneratorType &world::generator::GeneratorBuilder::setGeneratorType(
        world::generator::GeneratorType generatorType) {
...
bool GeneratorBuilder::checkConsistence() {
    bool any_of_disk = (path_.has_value()
            || (generatorType_.has_value() && generatorType_.value() == GeneratorType::FROM_THE_DISK));
    bool any_of_probabilities = (wallProbability_.has_value()
            || objectOnFloorProbability_.has_value()
            || NPCTypeProbability_.has_value()
            || artefactTypeProbability_.has_value());
